...
### 1. Cleanup
Run this if `Zephyr_Deps` was previously used as a submodule or if you are resetting the environment.
```bash
# Remove from Git index (if tracked)
git rm -r --cached Zephyr_Deps

# Clear internal Git module cache
rm -rf .git/modules/Zephyr_Deps
...
```bash
git clone git@github.com:imcase/Zephyr_Deps.git
```
...
* **NOTE**: run this command in the Parent Root-dir
```bash
git submodule add git@github.com:imcase/Zephyr_Deps.git
```
...
```bash
git add .
git commit -m "Development: Linked Zephyr_Deps submodule and created symlinks"
```
